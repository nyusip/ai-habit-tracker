...
try {
    ...
    .map(
    (h) =>
     `- ${h.name} (${h.category}, ${h. frequency}): completed ${h.completedDays} of the past 7 days, targat ${h.targetDays}/week`
    )
...
Completed: ${habitLogs.length}/30 days
Weekday Distribution:
Sun=${byDow[0]}, Mon=${byDow[1]}, Tue=${byDow[2]}, Wed=${byDow[3]}, Thu=${byDow[4]}, Fri=${byDow[5]}, Sat=${byDow[6]}
`;
      })
      .join("\n");
...
Please analyze the user's habits and answer the question with actionable insights.
`;
  ...
  try {
    ...
    const ctx = habits
      .map((h) => {
        ...
        return `${h.name}: current streak ${current} day${
          current !== 1 ? "s" : ""
        }`;
      })
      .join("\n");
    ...
    const todayLogs = logs.filter(
      (l) => todayKey(l.completedDate) === today
    );
...
Write a short, motivating morning message for the user.
Mention their current progress, celebrate any streaks,
encourage consistency, and suggest one priority habit
to focus on today. Keep the tone positive, supportive,
and actionable.
`;
